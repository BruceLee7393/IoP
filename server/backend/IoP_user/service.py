...
def user_to_dict(user):
    return {
        ...
        "created_at": format_datetime_to_utc_z(user.created_at),
        "updated_at": format_datetime_to_utc_z(user.updated_at),
    }
...
def update_user(user_id, payload):
    ...
    if "account" in payload:
        ...
        duplicated = User.query.filter(
            User.account == account, User.id != user.id
        ).first()
        if duplicated:
            raise InvalidUsageError("用户账号已存在")
    ...
    refreshed = (
        User.query.options(joinedload(User.role)).filter(User.id == user.id).first()
    )
