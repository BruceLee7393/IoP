...
def _validate_department_ids(department_ids):
    ...
    rows = (
        IodDepartment.query.with_entities(IodDepartment.id)
        .filter(IodDepartment.id.in_(department_ids))
        .all()
    )
...
def _get_user_department_map(user_ids):
    ...
    rows = (
        db.session.query(
            IodUserDepartment.user_id, IodDepartment.id, IodDepartment.dept_name
        )
...
def list_users(query_args):
    ...
    created_at_start = _parse_datetime_filter(
        query_args.get("created_at_start", type=str), "created_at_start"
    )
    created_at_end = _parse_datetime_filter(
        query_args.get("created_at_end", type=str), "created_at_end"
    )
    ...
    return {
        "items": [
            _user_to_dict(item, user_dept_map.get(item.id, [])) for item in paged.items
        ],
        "pagination": {
            "total": paged.total,
            "pages": paged.pages,
...
def create_user(payload):
    ...
    existed_account = (
        ...
        .filter(IodUser.user_id == account)
        .first()
    )
    if existed_account:
...
def update_user(user_id, payload):
    ...
    if "department_ids" in payload:
        ...
        IodUserDepartment.query.filter(IodUserDepartment.user_id == user.id).delete(
